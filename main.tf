...
module "route_table_modules" {
  ...
  internetGW_id = each.value.private == 0 ? module.internetGW_modules[each.value.gateway_name].internetGW_id : module.NatGW_modules[each.value.gateway_name].natGW_id
...
module "route_table_association_modules" {
  ...
  subnet_id = module.subnet_modules[each.value.subnet_name].subnet_id

  route_table_id = module.route_table_modules[each.value.route_table_name].route_table_id
}


module "NatGW_modules" {
  source = "./terraform_eks/modules/aws_natGW"

  for_each = var.nat_GW_config

  elasticIP_id = module.Elastic_IP_modules[each.value.eip_name].elastic_IP_id
  subnet_id  = module.subnet_modules[each.value.subnet_name].subnet_id
  tags = each.value.tags
}

module "Elastic_IP_modules" {
  source = "./terraform_eks/modules/aws_elastic_IP"
...
module "aws_eks" {
  source = "./terraform_eks/modules/aws_eks"
  for_each = var.aws_eks_cluster_config
  eks_cluster_name = each.value.eks_cluster_name
  subnet_ids = [module.subnet_modules[each.value.subnet1].subnet_id,
  module.subnet_modules[each.value.subnet2].subnet_id,
  module.subnet_modules[each.value.subnet3].subnet_id,
  module.subnet_modules[each.value.subnet4].subnet_id
  ]
  tags = each.value.tags
}

module "aws_eks_node_group" {
  source = "./terraform_eks/modules/aws_eks_node_group"
